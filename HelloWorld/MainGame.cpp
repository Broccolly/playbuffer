...
void UpdateAgent8()
{
	...
	if (!Play::IsVisible(obj_agent8) && gameState.agent8State == STATE_DEAD)
	{
		obj_agent8.pos = { -100, -100 }; // Added to stop Agent8 moving indefinitely when dead
		                                 // as it seemed to trigger collisions with coins incorrectly
		                                 // when far off the screen
	}
